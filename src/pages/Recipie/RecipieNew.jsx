...
import recipieNewEmpty from '../../assets/recipieNewEmpty.json';
        ...
        <main className="edit-recipie-container">
            ...
            <section className='content-width'>
                        ...
                        {recipieData.instructions.map((instruction, i) => (
                            <li className="instruction mg-3vh shadow" key={`${instruction}-${i}`}>
                                <InputGroup className={`instruction${i}`}>
                                    ...
                                    <Form.Control 
                                        className="instructions-textarea input-field" 
                                        as="textarea"
                    ...
                    <ul className='nutrition-container'>
                        {Object.entries(recipieData.nutrition).map(([key, value], i) => (
                            <li className="nutrition mg-3vh shadow" key={i}>
                                <InputGroup>
                                    <Form.Label htmlFor="nutrient" className='bold-text-sm'>{capitalizeWord(key)}</Form.Label>
                                    <div>
                                        <Form.Control
                                            className="nutrition-textarea input-field" id={`nutrient${i}`}
                                            defaultValue={value}
                                            onChange={(e) => handleIngredientChange(i, 'name', e.target.value)}
                                            placeholder={`${nutritionPlaceholders[i]}`}
                                        />
                                        <InputGroup.Text id="basic-addon2">{key === 'energy' ? 'kCal' : 'g'}</InputGroup.Text>
                                    </div>
                                </InputGroup>
                            </li>
                        ))}
                ...
                <div className='center-child'>
                    <button className='myBtns-primary mg-3vh wdth-100'>Create new recipie</button>
                </div>
